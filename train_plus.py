...
criterion_perctual=losses.PerceptualLoss().cuda()
######### DataLoaders ###########
...
for epoch in range(start_epoch, opt.OPTIM.NUM_EPOCHS + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        loss_perctual=sum([criterion_perctual(restored[j],target) for j in range(len(restored))])
        loss =   (0.05*loss_edge)+loss_perctual
